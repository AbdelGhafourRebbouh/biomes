...
// Register a system-wide hotkey to finish Phase 2
    ...
    if (e == EVENT_SYSTEM_MOVESIZESTART) {
        ...
        if (s_movingWindowHwnd == hwnd) {
            ...
            for (auto& b : s_savedBoxes) {
                RECT r = b.pixelRect; 
                OffsetRect(&r, s_monitors[b.monitorIndex].rect.left, s_monitors[b.monitorIndex].rect.top);
                if (PtInRect(&r, pt)) {
            ...
            for (auto& m : s_monitors) {
                SetWindowPos(m.hwndOverlay, HWND_BOTTOM, 0,0,0,0, SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);
                InvalidateRect(m.hwndOverlay, NULL, FALSE);
            }
        }
    }
...
void GridOverlay::DrawGrid(HDC hdc, HWND hwnd, int mIdx) {
    ...
    HBRUSH bB = CreateSolidBrush(s_isSnappingMode ? RGB(0, 0, 0) : s_theme.boxFillColor);
    HBRUSH hB = CreateSolidBrush(s_theme.boxHoverColor);
    HPEN bP = CreatePen(PS_SOLID, s_theme.boxPenWidth, s_theme.boxBorderColor);
    SelectObject(hdc, bP);
    ...
    for (auto& b : s_savedBoxes) {
        ...
        bool isHovered = (s_isSnappingMode && s_movingWindowHwnd && PtInRect(&sbr, m));
        SelectObject(hdc, isHovered ? hB : bB);
        if (s_isSnappingMode && !isHovered) {
        ...
        if (!b.assignedApp.empty()) {
            std::string n = b.assignedApp.substr(b.assignedApp.find_last_of("\\/") + 1);
            SetTextColor(hdc, RGB(255,255,255)); SetBkMode(hdc, TRANSPARENT);
            DrawTextA(hdc, n.c_str(), -1, &br, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        }
    }
    if (!s_isSnappingMode) {
        float cw = (float)r.right / s_cols, ch = (float)r.bottom / s_rows;
        HPEN p = CreatePen(PS_SOLID, 1, s_theme.gridLineColor); SelectObject(hdc, p);
        ...
        if (s_isDragging) { Rectangle(hdc, std::min(s_dragStart.x, s_dragCurrent.x), std::min(s_dragStart.y, s_dragCurrent.y), std::max(s_dragStart.x, s_dragCurrent.x), std::max(s_dragStart.y, s_dragCurrent.y)); }
        DeleteObject(p);
    }
...
LRESULT CALLBACK GridOverlay::WindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        ...
        case WM_HOTKEY:
            if (wp == HOTKEY_ID && s_isSnappingMode) {
        ...
        case WM_LBUTTONUP: if (s_isDragging) {
            ...
            float cw = (float)r.right/s_cols, ch = (float)r.bottom/s_rows;
            int sc = (int)(std::min(s_dragStart.x, s_dragCurrent.x)/cw), ec = (int)(std::max(s_dragStart.x, s_dragCurrent.x)/cw)+1;
            int sr = (int)(std::min(s_dragStart.y, s_dragCurrent.y)/ch), er = (int)(std::max(s_dragStart.y, s_dragCurrent.y)/ch)+1;
            SelectedBox b; b.id = (int)s_savedBoxes.size()+1; b.monitorIndex = mIdx;
        ...
        case WM_KEYDOWN: 
            if (wp == VK_RETURN && !s_isSnappingMode) {
                s_isSnappingMode = true;
                for (auto& m : s_monitors) {
                    // Register a global hotkey so Enter works even when focus is lost
                    RegisterHotKey(m.hwndOverlay, HOTKEY_ID, 0, VK_RETURN);
                    SetWindowPos(m.hwndOverlay, HWND_BOTTOM, 0,0,0,0, SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE|SWP_FRAMECHANGED);
